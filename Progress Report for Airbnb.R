...
# Applying One-hot encoding for room_type and neighbourhood_group
...
# Objective:
# The goal of this analysis is to understand the factors influencing Airbnb listing prices in New York City 
#and to develop predictive models for both price prediction and classification. This analysis focuses on key predictors,
#including room type, neighborhood group, and reviews, in order to forecast prices and classify listings into 
#low and high price categories.

# Data Preprocessing:
# The price column was converted to a numeric format, and any non-numeric entries were removed. 
# Missing values in the `reviews_per_month` column were imputed with the mean value for each room type. 
#Rows with missing values in either the price or reviews columns were excluded to ensure data integrity. 
# The `room_type` and `neighbourhood_group` variables were converted into categorical factors and one-hot encoded 
#for use in the regression models.

# Summary Statistics:
# The key statistics for the Airbnb dataset highlight considerable variability in the price and reviews data.
#The average price per listing was $233, with a median of $147, indicating the presence of a number of high-priced outliers. Prices ranged from a minimum of $8 to a maximum of $100,000, with a standard deviation of $1020. The average reviews per month was 1.24, with a median of 1.19, and a relatively low standard deviation of 1.89, indicating that most listings have few reviews. This variability suggests that factors such as room type, neighborhood, and listing quality may significantly impact the price.

# Linear Regression Analysis:
# A linear regression model was fitted to predict the price of Airbnb listings based on room type and neighborhood group. 
#The results revealed that "Entire home/apt" and "Hotel room" were the most significant predictors of higher prices,
#with increases of $158 and $274 respectively. On the other hand, "Private room" had a non-significant effect on price. 
# Listings located in "Manhattan" showed a significant positive effect on price, with an estimated increase of $158 compared to other neighborhoods.
#In contrast, neighborhoods like the Bronx and Queens did not show significant price differences.
...
#by room type and neighborhood group, indicating the presence of additional factors (such as location, amenities, 
#or availability) that likely contribute to price variations. 
# Residual analysis showed no clear pattern in the residuals, but some minor deviations from normality were noted 
#in the Q-Q plot, which suggests that the linear model may not fully capture all relationships in the data. 
#Furthermore, multicollinearity among the predictors may also affect the stability and accuracy of the model, 
#as several room types and neighborhood groups are likely correlated.

# Logistic Regression for Price Category Classification:
# To classify listings into "Low" or "High" price categories, a binary logistic regression model was used.
#The categories were defined based on the 33rd and 66th percentiles of price, with listings priced below
#the 33rd percentile categorized as "Low" and those above the 66th percentile categorized as "High."
# The logistic regression model revealed that room types such as "Entire home/apt" and "Hotel room" 
#were strongly associated with the high price category. Listings in "Manhattan" were also more likely to belong
#to the high price category, followed by "Brooklyn" and "Queens."
# The model achieved an Area Under the Curve (AUC) of 0.8348, indicating good discriminatory power between
#the low and high price categories. This suggests that the logistic regression model is an effective tool for 
#classifying listings based on price.

# Model Diagnostics and Multicollinearity:
# Residual analysis of the linear regression model revealed no major patterns, although minor deviations
#from normality were observed, suggesting that the model might not capture all the non-linear relationships in the data.
...
# Multicollinearity can cause instability in regression coefficients and affect model reliability.
#It is advisable to examine the Variance Inflation Factors (VIFs) for the predictors to better understand the degree of multicollinearity
#in the model.

# Next Steps:
# To improve the linear regression model, additional features such as amenities, listing availability, 
#and specific location data could be incorporated to capture more of the variance in price.
# Further exploration of advanced machine learning models like Random Forests or Gradient Boosting could
#help in addressing non-linear relationships and improving model accuracy.
# Cross-validation should be performed to assess the robustness and generalization ability of the models, 
#ensuring that they perform well on new, unseen data.
